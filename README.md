...
Fetches videos with search support

Displays loader, success, failure, and no-results states

Banner with dismiss option

Video navigation to details page
...
Like / Dislike toggle behavior

Save / Unsave video functionality

Saved videos persist across routes

💾 Saved Videos

Displays all saved videos

Shows empty state when no videos are saved
...
All data is fetched from predefined REST APIs:
...
(Endpoints and sample responses are included exactly as provided.)
...
Accessibility-compliant alt attributes

Loader wrapped with data-testid="loader"
...
API handling with loaders and error states
